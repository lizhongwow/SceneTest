...
namespace SceneTest
{
    public class grid_map
    {
        ...
        public IBaseUnit getSprite(int sprite_iid)
        {
            IBaseUnit u = null;
        ...
        public void rmv_player_from_map(IService game_ref, int clientid)
        {
            ...
            IBaseUnit spr = this.getSprite(iid);
        ...
        public static void update_keep_atk_range(IBaseUnit sprite, long tm_elasped_s)
        {
            ...
            if (gmap.has_sprite(pl.atking.tar_iid))
            {
        ...
        public static void update_pl_atk(IBaseUnit sprite, long cur_clock_tm)
        {
            ...
            if (!sprite.gmap.has_sprite(pl.atking.tar_iid))
            {
                // 目标不在同一地图
                sprite.cancel_atk();
                // TO DO : send msg
        ...
        public bool do_cast_target_skill(IBaseUnit caster, Variant rpc, bool precast = true, bool senderr = true)
        {
            ...
            IBaseUnit tar_sprite = this.getSprite(tar_iid);
            if (tar_sprite == null)
                return false;

            if (!this.has_sprite(caster.iid))
                return false;
        ...
        public bool do_cast_ground_skill(IBaseUnit caster, Variant rpc, bool precast = true, bool senderr = true)
        {
            ...
            {
                var tar_iid = rpc["to_iid"]._int32;
                tar_sprite = this.getSprite(tar_iid);
                if (tar_sprite == null)
                    return false;
        ...
        public static void update_jumping(IBaseUnit caster, IMapUnit pl, long cur_clock_tm)
        {
            if (pl.jumping == null)
                return;
            ...
            if (jumping.jump.trang != null)
            {
                Skill.apply_rang_eff(cur_clock_tm, caster, caster.get_pack_data(), jumping.jump.tres, jumping.jump.trang, skill_id, jumping.percent);
            }
            else
            {
                IBaseUnit tar_sprite = caster.gmap.getSprite(pl.jumping.tar_iid);
                if (tar_sprite != null)
                {
                    var tar_pl = tar_sprite.get_pack_data();
        ...
        public static void update_teleping(IBaseUnit caster, IMapUnit pl, long cur_clock_tm)
        {
            if (pl.teleping == null)
            {
                return;
